...
function setCombobox(bMethod) {
	combos=getElementsByClassName('select', "comboBox");
 	for(i=0; i<combos.length; i++) {
 	    nSizes = Element.Methods.positionedOffset(combos[i]);
...
function inittextfield(ctrl) {
	...
	textfield.value = "";
	textfield.style.lineHeight="inherit";
	...
	initextvalue = ctrl.options[ctrl.selectedIndex].value; 
	if (initextvalue == "") 
	{ 
	    textfield.value = ""; 
	    textfield.style.color = "#ccc"; 
	} else { 
	    textfield.value = ctrl.options[ctrl.selectedIndex].value;
	}    
